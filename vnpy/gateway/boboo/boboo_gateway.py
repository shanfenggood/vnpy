"""
Gateway for Boboo Crypto Exchange.
"""
...
ORDERTYPE_BOBOO2VT = {v: k for k, v in ORDERTYPE_VT2BOBOO.items()}

DIRECTION_VT2BOBOO = {
    Direction.LONG: "BUY",
    Direction.SHORT: "SELL"
}
DIRECTION_BOBOO2VT = {v: k for k, v in DIRECTION_VT2BOBOO.items()}
...
class BobooGateway(BaseGateway):
    """
    VN Trader Gateway for Boboo connection.
    """
    ...
    exchanges = [Exchange.BOBOO]

    def __init__(self, event_engine):
        """Constructor"""
        super().__init__(event_engine, "BOBOO")

        self.trade_ws_api = BobooTradeWebsocketApi(self)
        self.market_ws_api = BobooDataWebsocketApi(self)
        self.rest_api = BobooRestApi(self)
...
class BobooRestApi(RestClient):
    """
    Boboo REST API
    """

    def __init__(self, gateway: BobooGateway):
        """"""
        super().__init__()
    ...
    def query_contract(self):
        ...
        self.add_request(
            method="GET",
            path="/openapi/v1/brokerInfo?type=token",
    ...
    def send_order(self, req: OrderRequest):
        ...
        params = {
            "symbol": req.symbol,
            "timeInForce": "GTC",
            "side": DIRECTION_VT2BOBOO[req.direction],
            "type": ORDERTYPE_VT2BOBOO[req.type],
            "price": str(req.price),
    ...
    def on_query_order(self, data, request):
        ...
        for d in data:
            ...
            order = OrderData(
                ...
                symbol=d["symbol"],
                exchange=Exchange.BOBOO,
                price=float(d["price"]),
                volume=float(d["origQty"]),
                type=ORDERTYPE_BOBOO2VT[d["type"]],
                direction=DIRECTION_BOBOO2VT[d["side"]],
                traded=float(d["executedQty"]),
                status=STATUS_BOBOO2VT.get(d["status"], None),
                time=time,
                gateway_name=self.gateway_name,
            )
            self.gateway.on_order(order)
    ...
    def on_query_contract(self, data, request):
        ...
        self.gateway.write_log("现货信息查询成功")
    ...
    def query_history(self, req: HistoryRequest):
        ...
        while True:
            ...
            params = {
                "symbol": req.symbol,
                "interval": INTERVAL_VT2BOBOO[req.interval],
                "limit": limit,
...
class BobooTradeWebsocketApi(WebsocketClient):
    """"""
    ...
    def on_order(self, packet: dict):
        ...
        order = OrderData(
            ...
            exchange=Exchange.Boboo,
            orderid=orderid,
            type=ORDERTYPE_Boboo2VT[packet["o"]],
            direction=DIRECTION_Boboo2VT[packet["S"]],
            price=float(packet["p"]),
            volume=float(packet["q"]),
            traded=float(packet["z"]),
            status=STATUS_Boboo2VT[packet["X"]],
            time=time,
            gateway_name=self.gateway_name
        )
...
class BobooDataWebsocketApi(WebsocketClient):
    """"""
